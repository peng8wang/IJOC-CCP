...
for N = [500, 1000, 1500, 2000]
    for alpha = [0.05, 0.1]    % risk level
        ...
        %% choose the method
        IP = 1; CVa = 1; DC = 1; pDC = 1; pDC1 = 1; pDC2 = 1; AL = 1; SC = 1; repeat_num = 5;
        ...
        [iter_PD_collect, iter_PD_collect1, iter_PD_collect2, iter_DC_collect, iter_AL_collect, iter_SC_collect, iter_SC1_collect]= deal(zeros(repeat_num,1));
        [fval_MIP_collect, fval_CVaR_collect, fval_DC_collect, fval_PD_collect, fval_PD_collect1, fval_PD_collect2, fval_AL_collect, fval_SC_collect, fval_SC1_collect] = deal(zeros(repeat_num,1));
        [prob_MIP_collect, prob_CVaR_collect, prob_DC_collect, prob_PD_collect, prob_PD_collect1, prob_PD_collect2, prob_AL_collect, prob_SC_collect, prob_SC1_collect] = deal(zeros(repeat_num,1));
        [time_MIP_collect, time_CVaR_collect, time_DC_collect, time_PD_collect, time_PD_collect1, time_PD_collect2, time_AL_collect, time_SC_collect, time_SC1_collect] = deal(zeros(repeat_num, 1));
        [ttime_MIP_collect, ttime_CVaR_collect, ttime_DC_collect, ttime_PD_collect, ttime_PD_collect1, ttime_PD_collect2, ttime_AL_collect, ttime_SC_collect, ttime_SC1_collect] = deal(zeros(repeat_num, 1));
        ...
        for iter = 1:repeat_num
            ...
            %% solve MIP by Gurobi
            if IP == 1
                opts = struct('solver', solver, 'maxitime', maxitime); upper = 1e3;
                tic; [x_MIP,time_MIP] = MIP(train_samples, C, a, theta, upper, alpha, opts); ttime_MIP = toc;
                fval_MIP = trace(C'*x_MIP)+trace(a'*x_MIP.^2); prob_MIP = risk_level(train_samples, x_MIP, m);
            ...
            if CVa == 1
                ...
                fval_CVaR = trace(C'*x0)+trace(a'*x0.^2); prob_CVaR = risk_level(train_samples, x0, m);
            ...
            if DC == 1
                ...
                tic; [x_DC, time_DC, iter_DC] = DCA(train_samples, C, a, theta, alpha, opts); ttime_DC = toc;
                fval_DC = trace(C'*x_DC)+trace(a'*x_DC.^2); prob_DC = risk_level(train_samples, x_DC, m);
            ...
            if pDC == 1
                ...
                tic; [x_PD, time_PD, iter_PD] = pDCA(train_samples, C, a, theta, alpha, beta, opts); ttime_PD = toc;
                fval_PD = trace(C'*x_PD)+trace(a'*x_PD.^2); prob_PD = risk_level(train_samples, x_PD, m);
            ...
            if pDC1 == 1
                ...
                tic; [x_PD1, time_PD1, iter_PD1] = pDCA(train_samples, C, a, theta, alpha, beta, opts); ttime_PD1 = toc;
                fval_PD1 = trace(C'*x_PD1)+trace(a'*x_PD1.^2); prob_PD1 = risk_level(train_samples, x_PD1, m);
            ...
            if pDC2 == 1
                ...
                tic; [x_PD2, time_PD2, iter_PD2] = pDCA(train_samples, C, a, theta, alpha, beta, opts); ttime_PD2 = toc;
                fval_PD2 = trace(C'*x_PD2)+trace(a'*x_PD2.^2); prob_PD2 = risk_level(train_samples, x_PD2, m);
            ...
            if AL == 1
                ...
                tic; [x_AL, time_AL, iter_AL] = ALDM(train_samples, C, a, theta, alpha, opts); ttime_AL = toc;
                fval_AL = trace(C'*x_AL)+trace(a'*x_AL.^2); prob_AL = risk_level(train_samples, x_AL, m);
            ...
            if SC == 1
                eps = 1e-8;
                tic; [x_SC, time_SC, iter_SC] = SCA(train_samples, C, a, theta, alpha, eps, opts); ttime_SC = toc;
                fval_SC = trace(C'*x_SC)+trace(a'*x_SC.^2); prob_SC = risk_level(train_samples, x_SC, m);
                x_SC_collect{iter} = x_SC; fprintf('SCA %f\n', fval_SC);
        ...
        %% print information
        fid = fopen(['CCSCP_date',datestr(now,'mm-dd_HH-MM'),'_a=',num2str(alpha),'_N=',num2str(N),'_m=',num2str(m),'n=',num2str(n),'.txt'],'wt');
        ...
        fclose(fid);
        save(strcat('CCSCPdate',datestr(now,'mm-dd_HH-MM'),'_N=',num2str(N)));
    end
end