function [x, time, iter] = pDCA(S, c, theta, alpha, beta, opts)
...
for iter = 1 : maxiter
    ...
    if strcmp(solver,'gurobi') == 1
        ...
        model.obj = [c1-beta*xk(:)' zeros(1,2*N+1)];
        model.Q = [(beta/2)*speye(n*m) sparse(n*m,2*N+1);sparse(2*N+1,n*m) sparse(2*N+1,2*N+1)];
        ...
        %% Define an objective
        Objective = trace(c'*x) + beta/2*norm(x-xk,'fro')^2;
    ...
    fval = trace(c'*xk);
    prob = risk_level(S, xk, m);

    fprintf('Iternum: %d, fval: %.4f, r: %.6f, t: %.1f, p: %.4f\n', iter, fval, abs(fval - fval_old)/max(1,abs(fval_old)), time, prob);
    beta = max(beta/4,1e-3);
